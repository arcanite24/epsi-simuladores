...
export class PaymentModelPanelComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.guias$ = this.afs
      .collection<PaymentModel>(Collections.PAYMENT_MODEL, (ref) =>
        ref.where("type", "==", PaymentModelType.Guia)
      )
      .valueChanges()
      .pipe(
        tap((models) => {
    ...
    this.apuntes$ = this.afs
      .collection<PaymentModel>(Collections.PAYMENT_MODEL, (ref) =>
        ref.where("type", "==", PaymentModelType.Apunte)
      )
      .valueChanges()
      .pipe(
        tap((models) => {
    ...
    this.simuladores$ = this.afs
      .collection<PaymentModel>(Collections.PAYMENT_MODEL, (ref) =>
        ref.where("type", "==", PaymentModelType.Simulador)
      )
      .valueChanges()
      .pipe(
        tap((models) => {
  ...
  get total(): number {
    ...
    if (this.apuntes && this.totalSelectedApuntes >= 7) {
      total *= 0.9;
    }
  ...
  getGuiasTotal(guias: PaymentModel[] = [], selected: any = {}) {
    return +parseFloat(
      guias
        .filter((guia) => selected[guia.id])
        .map((guia) => +guia.amount)
        .reduce((a, b) => a + b, 0)
        .toString()
    );
  }
  ...
  async generatePayment(model: PaymentModel) {
    if (this.loading) {
      return this.toastr.error("No puedes generar otra solicitud de pago...");
    }
    this.loading = true;
    ...
    const email =
      environment.production === true
        ? user.email
        : "test_user_41665327@testuser.com";

    try {
      if (this.selectedStat) {
        await this.afs
          .collection(Collections.Stat)
          .doc(this.selectedStat)
      ...
      if (this.coupon && this.coupon.length > 5) {
        const discount = await this.afs
          .collection<Coupon>(Collections.COUPON, (ref) =>
            ref.where("code", "==", this.coupon)
          )
          .valueChanges()
          .pipe(
            take(1),
      ...
      const selectedSim = this.simuladores.filter(
        (s) => s.id === this.simuladorSelection
      )[0];

      const extraUnlock = flattenDeep([
        ...this.guias
          .filter((guia) => this.guiaSelection[guia.id] === true)
          .map((g) => g.unlocks),
        ...this.apuntes
          .filter((apunte) => this.apunteSelection[apunte.id] === true)
          .map((g) => g.unlocks),
      ...
      await this.afs
        .collection(Collections.ExtraUnlock)
        .doc(extraUnlockPayload.id)
        .set({ ...extraUnlockPayload });
      ...
      if (isFullCoupon) {
        ...
        console.log({ ...user });
        const _user = await this.afs
          .doc(`${Collections.USER}/${user.uid}`)
          .valueChanges()
          .pipe(take(1))
          .toPromise();
        ...
        await this.afs
          .doc(`${Collections.COUPON}/${request_payload.coupon}`)
          .update({
      ...
      if (request_payload.coupon) {
        amount -= amount * request_payload.coupon_value;
      }
      ...
      console.log(error);
      this.toastr.error(
        "Ocurrió un error al generar tu pago, por favor intenta más tarde o contacta con un administrador."
      );
      this.loading = false;
    }
  }
  ...
  async loadPaypalButton(model: PaymentModel) {
    ...
    if (isFullCoupon) {
      ...
      return this.toastr.error(
        "El cupón es del 100%, no uses PayPal para redimir el cupón."
      );
    }

    const amount = request_payload.coupon_value
      ? model.amount - model.amount * request_payload.coupon_value
      : model.amount;

    this.payment.generatePaypalButton(
      `paypal-container-${model.id}`,
      amount,
      async (payment) => {
        console.log(model);
