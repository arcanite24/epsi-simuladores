...
export class ExamQuestionsImportComponent implements OnInit {
  ...
  formatCsv(data: any[]): any[] {
    return data.map((q) => {
      ...
      if (correcta === "1") {
        payload.correcta = respuestas[0].id;
      }
      if (correcta === "2") {
        payload.correcta = respuestas[1].id;
      }
      if (correcta === "3") {
        payload.correcta = respuestas[2].id;
      }
      if (correcta === "4") {
        payload.correcta = respuestas[3].id;
      }
  ...
  async writeToDB(questions: Question[]) {
    ...
    await this.afs
      ...
      .doc(exam.id)
      .update({ questions: exam.questions });
