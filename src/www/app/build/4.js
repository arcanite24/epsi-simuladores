...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__home__ = __webpack_require__(1219);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__ionic_native_in_app_browser__ = __webpack_require__(593);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__ionic_native_network__ = __webpack_require__(596);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_angularfire2_auth__ = __webpack_require__(602);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10_angularfire2_firestore__ = __webpack_require__(589);
...
let HomePage = class HomePage {
    constructor(navCtrl, navParams, back, toast, modal, iap, network, afAuth, auth, data, afs) {
        this.navCtrl = navCtrl;
    ...
    ionViewDidLoad() {
        ...
        this.users$ = this.afs.collection(__WEBPACK_IMPORTED_MODULE_8__app_app_models__["a" /* Collections */].USER, ref => ref
            .where('promedio', '>', 0)
    ...
    sendOfflineResults() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const key of keys) {
                if (key.includes('result-offline-')) {
                    const result = JSON.parse(localStorage.getItem(key));
                    const id = key.replace('result-offline-', '');
                    yield this.afs.doc(`${__WEBPACK_IMPORTED_MODULE_8__app_app_models__["a" /* Collections */].EXAM_RESULT}/${id}`).set(result);
                    this.toast.create({ message: 'Resultados offline enviados correctamente', duration: 4000 });
                }
            }
        });
    }
    loadMoodRates(uid) {
        return __awaiter(this, void 0, void 0, function* () {
            const rates = yield this.data.getCollectionQuery(__WEBPACK_IMPORTED_MODULE_8__app_app_models__["a" /* Collections */].MOOD_RATE, ref => ref.where('user', '==', uid));
            /* const dailyRegisters = await this.data.getCollectionQuery<MoodRate>(Collections.DAILY_REGISTER, ref => ref.where('user', '==', uid)) */
            const today = __WEBPACK_IMPORTED_MODULE_9_moment___default()().format('DD-MM-YYYY');
    ...
    openNotis() {
        const m = this.modal.create('NotificationsPage');
        m.present();
        m.onDidDismiss(data => this.loadNotis());
    }
    loadNotis() {
        this.back.getNotisUser(this.uid).subscribe(data => {
            this.notRead = data.filter(n => !n.read).length;
        }, err => /* this.toast.create({message: 'No se pudo cargar la cantidad de notificaciones no leídas...', duration: 2000}).present() */ null);
    }
    checkForInternet() {
        this.network.onDisconnect().subscribe(() => {
    ...
    openVideos(isPrograma = false) {
        // EspecialidadesPage
        if (this.auth.isTemprano)
            return this.navCtrl.push('EspecialidadesPage', { isPrograma });
    ...
    purge() {
        this.back.purgeGoogle().subscribe(data => console.log(data), err => console.log(err));
    }
    openCalendar() {
